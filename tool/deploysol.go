...
func main() {
	...
	flag.StringVar(&port, "port", "22222", "port地址")
	flag.Int64Var(&timeSleep, "sleep", 0, "ip地址")

	flag.Parse()

	client, err := eth.Connect("http://" + ip + ":" + port)
	...
	go checkNonce(n, stop) //每分钟检查nonce
	...
	for {
		...
		case <-stop:
			...
			fmt.Println("出错后查询的nonce是", nonce)
...
func nonceNum(n *uint64) {
	...
	for {
		...
		fmt.Println("每秒发送多少交易", atomic.LoadUint64(n)-n1)

		n1 = *n
	}
}

func checkNonce(nonce *uint64, stop chan uint64) {
	var pre uint64

	for {
		time.Sleep(1 * time.Minute)

		if atomic.LoadUint64(nonce) == pre {

			//如果nonce没有变化,停止交易
			stop <- 1

		} else {
			pre = atomic.LoadUint64(nonce)
		}
