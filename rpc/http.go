...
func DialHTTPWithClient(endpoint string, client *http.Client, token string) (*Client, error) {
...
req.Header.Set("X-PDX-PROXY-JWT", token)//todo 大小写
...
// note: proxy[0]:url, proxy[1]:token
...
log.Debug("http client setting proxy", "proxy", proxy[0], "token", proxy[1])
tr := &http.Transport{
...
return DialHTTPWithClient(endpoint, httpClient, token)
}
