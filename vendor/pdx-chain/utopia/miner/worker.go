...
func newUtopiaWorker(config *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, recommit time.Duration) *utopiaWorker {
	utopiaWorker := &utopiaWorker{
		...
		newWorkCh:      make(chan *utopia_types.BlockMiningReq, 10000),
	...
	utopiaEngine.Running = &utopiaWorker.running
...
func (w *utopiaWorker) resultLoop() {
	for {
		...
		case result := <-w.resultCh:
			...
			timeEnd := float64(time.Duration(utopia_engine.BlockDelay))
...
func (w *utopiaWorker) initCurrentMStateDB(num int) error {
...
func (w *utopiaWorker) commitTransaction(tx *types.Transaction, coinbase common.Address, rmap map[common.Hash]*types.Receipt, idx int) ([]*types.Log, error) {
...
func (w *utopiaWorker) commitTransactions(txs *types.TransactionsByPriceAndNonce,
	...
	for {
		...
		case pdxcc.ErrChainCodeTimeOut:
...
func (w *utopiaWorker) commitNewWork(work *utopia_types.BlockMiningReq, timeMain time.Time) {
	...
	if work != nil {
		...
		case utopia_types.NORNAML_BLOCK:
			extra = utopia_types.BlockExtra{NodeID: work.NodeID, Rank: work.Rank, CNumber: work.CNumber, IsLandID: work.IsLandID, Empty: work.Empty}
	...
	rewardAddr := utopia.Config.String("rewardAddress")
...
func (w *utopiaWorker) fillTxsToBuckets(txs map[common.Address]types.Transactions) []map[common.Address]types.Transactions {
