...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "utopia-android-")
	if err != nil {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "utopia-chain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "utopia.aar"), "utopia.aar", os.ModePerm)
