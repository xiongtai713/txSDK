...
Building Swarm requires Go (version 1.10 or later).
    ...
    go install pdx-chain/cmd/swarm
...
You must have your working copy under `$GOPATH/src/pdx-chain`.
...
```
git clone git@github.com:nirname/go-ethereum.git $GOPATH/src/pdx-chain
```
...
```
stateth --rm --grafana-dashboards-folder $GOPATH/src/pdx-chain/swarm/grafana_dashboards --influxdb-database metrics
```
 ...
 * [Code review guidelines](https://pdx-chain/wiki/Code-Review-Guidelines).
