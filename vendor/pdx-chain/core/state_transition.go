...
var utopiaExtraFlag = util.EthHash([]byte("utopia"))
...
func (st *StateTransition) TransitionDb(extra map[string][]byte) (ret []byte, usedGas uint64, failed bool, err error) {
	...
	//parse data
	var meta utils.Meta
	st.data, meta, err = utopia.ParseData(st.data)
	if err != nil {
	...
	if contractCreation {
		...
		/*if vm.EwasmFuncs.IsWASM(final) && meta != nil && err == nil {
			final, err = utopia.AssemblePayload(final, meta)
			if err != nil {
	...
	if vmerr != nil {
		...
		if vmerr == vm.ErrInsufficientBalance || vmerr == pdxcc.ErrChainCodeTimeOut {
