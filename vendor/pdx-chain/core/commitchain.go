...
func NewCommitChain(blockChain *BlockChain) *CommitChain {
	...
	if currentBlock != nil && currentBlock.NumberU64() != 0 && blockChain.CurrentBlock().NumberU64() < commitExtra.NewBlockHeight.Uint64() {
		...
		if blockChain.CurrentBlock().NumberU64() == 0 {
			log.Info("NewCommitChain", "currentBlock", 1)
			cnum = 1
		} else {
			//normal对应的commit高度
			log.Info("从启后要从那个normal开始", "高度", blockChain.CurrentBlock().NumberU64())
			extra := utopia_types.BlockExtraDecode(blockChain.CurrentBlock())
...
func (cc *CommitChain) ClearnUpHeightMap(normalBlockHeight uint64) {
	cc.heightmu.Lock()
	defer cc.heightmu.Unlock()
...
func (sc *CommitChain) loadLastState() error {
	...
	if head == (common.Hash{}) {
		...
		if normalBlock.NumberU64() <= 1 {
			...
			extra := utopia_types.BlockExtraDecode(normalBlock)
